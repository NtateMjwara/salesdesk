...
<div class="car-grid">
  ...
  <div class="car-card <?= $isSold ? 'car-sold' : ($isPaused ? 'car-paused' : '') ?>">
    ...
    <div class="car-card-body">
      ...
      <div class="share-sheet" id="share-<?= $car['id'] ?>" style="display:none">
        <div class="share-url-row">
          <input type="text" class="finput" value="<?= htmlspecialchars($shareUrl) ?>"
                 id="shareUrl-<?= $car['id'] ?>" readonly style="font-size:11px;font-family:var(--mono)">
          <button class="btn btn-ghost btn-sm" onclick="copyShare(<?= $car['id'] ?>)">
            <i class="fa-solid fa-copy"></i>
          </button>
        </div>
        <a href="https://wa.me/?text=<?= urlencode('Check out this ' . $car['year'].' '.$car['make'].' '.$car['model'] . ' for R'.number_format((float)$car['price'],0,'.',',' ) . ': ' . $shareUrl) ?>"
           target="_blank" rel="noopener"
           class="btn btn-success btn-sm" style="width:100%;margin-top:6px;background:#25D366;border:none">
          <i class="fa-brands fa-whatsapp"></i> Share on WhatsApp
        </a>
      </div>

      <div class="car-card-actions">
        <button class="btn btn-primary btn-sm" onclick="toggleShare(<?= $car['id'] ?>)">
