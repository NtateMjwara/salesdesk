...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'publish') {
        // Generate slug: year-make-model-colour, deduplicated.
        ...
        unset($_SESSION['car_wz']);
        $_SESSION['flash_ok'] = "{$wz['year']} {$wz['make']} {$wz['model']} listed successfully.";
...
// ── Lists for dropdowns ────────────────────────────────────────
$makes        = ['Alfa Romeo','BMW','Chevrolet','Chrysler','Citroën','Datsun','Ferrari','Fiat','Ford','GWM','Honda','Hyundai','Isuzu','Jaguar','Jeep','Kia','Land Rover','Lexus','Mahindra','Mazda','Mercedes-Benz','MG','Mini','Mitsubishi','Nissan','Opel','Peugeot','Porsche','Range Rover','Renault','Rolls-Royce','SEAT','Ssangyong','Subaru','Suzuki','Toyota','Volkswagen','Volvo'];
...
$commPreview = '';
if ($wz['price'] > 0 && $wz['commission_value'] > 0) {
    if ($wz['commission_type'] === 'fixed') {
        $commPreview = 'R ' . number_format($wz['commission_value'], 0);
    } else {
        $commPreview = 'R ' . number_format($wz['price'] * $wz['commission_value'] / 100, 0)
            . ' (' . $wz['commission_value'] . '% of R ' . number_format($wz['price'], 0) . ')';
    }
}
    ...
    <div class="frow">
      <div class="fgroup">
        ...
        <input class="finput" type="number" id="mileage" name="mileage"
               min="0" step="1000" placeholder="28000"
  ...
  <form method="POST" enctype="multipart/form-data">
    ...
    <label for="images"
      ...
      <input type="file" id="images" name="images[]"
             ...
             onchange="this.closest('label').querySelector('span').textContent = this.files.length + ' file(s) selected'">
    </label>
  ...
  <form method="POST" id="commForm">
    ...
    <div class="fgroup">
      ...
      <input class="finput" type="number" id="commission_value" name="commission_value"
             ...
             value="<?= $wz['commission_value'] > 0 ? $wz['commission_value'] : '' ?>"
...
<div class="card card-body">
  ...
  <!-- Cover image -->
  <?php if (!empty($wz['image_urls'][0])): ?>
  <div style="border-radius:var(--r-md);overflow:hidden;margin-bottom:1.25rem;
              height:200px;background:var(--bg);">
    <img src="<?= htmlspecialchars($wz['image_urls'][0]) ?>" alt="Cover"
         style="width:100%;height:100%;object-fit:cover;">
  </div>
  <?php endif; ?>
