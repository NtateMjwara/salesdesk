 ...
 *
 * REFACTORED: The page-scoped <style> block (previously inline in this file)
 * has been removed entirely and replaced with the shared semantic classes
 * already defined in dealer.css §9 (Team management cards) plus the
 * additions in dealer-team-patch.css. This was the single biggest cause of
 * the mobile overflow: the old scoped CSS had its own grid-template-columns
 * for .exec-card with no matching media query, so it never collapsed below
 * 1024px. The shared .d-exec-card classes already have the full responsive
 * treatment (3-col → 2-col → stacked actions) defined in dealer.css.
 */
...
<div class="dash-header">
  <div class="dash-header__text">
    <h1 class="page-header__title">Your <em>team</em></h1>
    <p class="dash-header__sub-plain"><?= htmlspecialchars($dealerName) ?> — sales executives</p>
  </div>
</div>

<!-- Summary strip -->
<div class="d-team-stat-strip">
  <div class="d-team-stat-box">
    <div class="d-team-stat-box__num"><?= $counts['verified'] ?></div>
    <div class="d-team-stat-box__label">Active execs</div>
  </div>
  <div class="d-team-stat-box">
    <div class="d-team-stat-box__num" style="<?= $counts['pending'] ? 'color:var(--amber)' : '' ?>"><?= $counts['pending'] ?></div>
    <div class="d-team-stat-box__label">Pending approval</div>
  </div>
  <div class="d-team-stat-box">
    <div class="d-team-stat-box__num"><?= $counts['suspended'] ?></div>
    <div class="d-team-stat-box__label">Suspended</div>
  </div>
  <div class="d-team-stat-box">
    <div class="d-team-stat-box__num"><?= $counts['rejected'] ?></div>
    <div class="d-team-stat-box__label">Declined</div>
  </div>
</div>

<?php if (empty($roster)): ?>
<div class="empty">
  <span class="empty-icon"><i class="fa-solid fa-user-tie" aria-hidden="true"></i></span>
...
if ($pending):
?>
<div class="d-team-section-head">
  <span class="d-team-section-label">Pending approval</span>
  <span class="d-team-section-count d-team-section-count--alert"><?= count($pending) ?></span>
</div>
<?php foreach ($pending as $exec):
  $name = trim(($exec['first_name'] ?? '') . ' ' . ($exec['last_name'] ?? '')) ?: 'Unnamed';
?>
<div class="d-exec-card d-exec-card--pending">
  <div class="avatar"><?= initials($name) ?></div>
  <div class="d-exec-card__info">
    <div class="d-exec-card__name">
      <?= htmlspecialchars($name) ?>
      <?= statusBadge('pending') ?>
    </div>
    <div class="d-exec-card__meta">
  ...
  <div class="d-exec-card__actions">
    <!-- Approve -->
...
?>
<div class="d-team-section-head">
  <span class="d-team-section-label">Active team</span>
  <span class="d-team-section-count"><?= count($verified) ?></span>
</div>
<?php foreach ($verified as $exec):
  $name = trim(($exec['first_name'] ?? '') . ' ' . ($exec['last_name'] ?? '')) ?: 'Unnamed';
?>
<div class="d-exec-card d-exec-card--verified">
  <div class="avatar"><?= initials($name) ?></div>
  <div class="d-exec-card__info">
    <div class="d-exec-card__name">
      <?= htmlspecialchars($name) ?>
    ...
    <div class="d-exec-card__stats">
      <div class="d-exec-card__stat"><strong><?= (int)$exec['active_cars'] ?></strong> active listings</div>
      <div class="d-exec-card__stat"><strong><?= (int)$exec['total_leads'] ?></strong> leads</div>
      <?php if ($exec['verified_at']): ?>
      <div class="d-exec-card__stat">Active since <?= date('j M Y', strtotime($exec['verified_at'])) ?></div>
      <?php endif; ?>
    </div>
  </div>
  <div class="d-exec-card__actions">
    <button class="btn btn-warn btn-sm" type="button"
      ...
      <i class="fa-solid fa-pause" aria-hidden="true"></i> Suspend
    </button>
  </div>
</div>
<?php endforeach; ?>
...
?>
<div class="d-team-section-head">
  <span class="d-team-section-label">Suspended</span>
  <span class="d-team-section-count"><?= count($suspended) ?></span>
</div>
<?php foreach ($suspended as $exec):
  $name = trim(($exec['first_name'] ?? '') . ' ' . ($exec['last_name'] ?? '')) ?: 'Unnamed';
?>
<div class="d-exec-card d-exec-card--suspended">
  ...
  <div class="d-exec-card__info">
    ...
    <div class="d-reason-note">
      <strong>Reason:</strong> <?= htmlspecialchars($exec['rejection_reason']) ?>
    </div>
    <?php endif; ?>
    <div class="d-exec-card__stats">
      <div class="d-exec-card__stat"><strong><?= (int)$exec['total_cars'] ?></strong> total listings</div>
      <div class="d-exec-card__stat"><strong><?= (int)$exec['total_leads'] ?></strong> leads</div>
    </div>
  </div>
  <div class="d-exec-card__actions">
    ...
    <form method="POST" style="margin:0;">
      ...
      <button class="btn btn-info btn-sm" type="submit">
        <i class="fa-solid fa-rotate-left" aria-hidden="true"></i> Reinstate
      </button>
    </form>
  </div>
</div>
...
?>
<div class="d-team-section-head">
  <span class="d-team-section-label">Declined</span>
  <span class="d-team-section-count"><?= count($rejected) ?></span>
</div>
<?php foreach ($rejected as $exec):
  $name = trim(($exec['first_name'] ?? '') . ' ' . ($exec['last_name'] ?? '')) ?: 'Unnamed';
?>
<div class="d-exec-card d-exec-card--rejected">
  ...
  <div class="d-exec-card__actions">
    ...
    <form method="POST" style="margin:0;">
      ...
      <input type="hidden" name="exec_id" value="<?= $exec['id'] ?>">
      <button class="btn btn-info btn-sm" type="submit">
        <i class="fa-solid fa-rotate-left" aria-hidden="true"></i> Approve
      </button>
    </form>
  </div>
</div>
