 ...
 *           Previously: set step=99, redirect → register.php → guard fires
 *           on !empty($_SESSION['user_id']) → redirect('/app/dashboard.php')
 *           — a generic path that does not exist, causing a 404.
 *           The step-99 success screen is now only shown for sales_exec
 *           (who are NOT logged in yet and need to see the pending-approval
 *           message). Brokers and dealers skip it and land on their dashboard.
 *
 *   FIX-06: Role-scoped record activation in finaliseOnboarding().
 *
 *   FIX-SKIP-01: wizard_skip_dealership was a GET-based action that mutated
 *           $_SESSION['wz'] before validateCSRF() was called, meaning any
 *           page could link to it and force an exec to skip dealership
 *           selection. Converted to POST-only, moved after validateCSRF(),
 *           and extracted into handleWizardSkipDealership(). Added session
 *           guard and isSalesExec() role check.
 *
 *   FIX-SKIP-02: wizard_skip_address had the same pre-CSRF GET mutation
 *           pattern as wizard_skip_dealership. Converted to POST-only,
 *           moved after validateCSRF(), and extracted into
 *           handleWizardSkipAddress(). Added session guard.
 */
...
// wizard_reset is the only remaining GET action — it only destroys
// the wizard session and redirects, which is safe without CSRF because
// the worst an attacker can do is reset the user's own in-progress wizard.
...
// All remaining actions require a POST with a valid CSRF token.
...
//
// FIX-SKIP-01: Previously handled as a GET action before validateCSRF(),
// allowing any page to force an exec to skip via a crafted link. Now POST-
// only and validated. Guards added for session integrity and role check.
// ═══════════════════════════════════════════════════════════
function handleWizardSkipDealership(): never
{
    ...
    // Only sales execs have a dealership step to skip. Any other role
    // landing here is either a misconfigured form or a forged request.
    if (!isSalesExec()) {
        redirect('/auth/register.php');
    }
...
// STEP 4/5 — Password
//
// FIX-03: Use PASSWORD_ALGO constant (Argon2id) instead of hardcoded
//         PASSWORD_BCRYPT to honour config.php's security decision.
// ═══════════════════════════════════════════════════════════
function handleWizardPassword(): never
{
    ...
    // FIX-03: PASSWORD_ALGO (Argon2id) instead of PASSWORD_BCRYPT.
...
//
// FIX-SKIP-02: Previously handled as a GET action before validateCSRF(),
// allowing any page to force a user past the address step via a crafted
// link. Now POST-only and validated. Session guard added.
// ═══════════════════════════════════════════════════════════
...
//
// FIX-07: No longer bounces through register.php step 99.
//         The step-99 screen is preserved only for sales_exec
//         (pending-approval message, not auto-logged-in).
//         Brokers and dealers go directly to their dashboards.
// ═══════════════════════════════════════════════════════════
function finaliseOnboarding(): never
{
    ...
    // sales_exec: no desk or dealer record to activate here.
    ...
    // Sales exec: NOT auto-logged in. They stay on the pending-approval screen.
    if ($role === 'sales_exec') {
        $_SESSION['wz']['step'] = 99;
        redirect('/auth/register.php');
    }

    // Broker / dealer: log in and go straight to their dashboard.
    // FIX-07: Do NOT redirect to register.php. That page's logged-in guard
    //         sends them to the hardcoded '/app/dashboard.php' (does not exist).
    ...
    if ($user) {
        ...
        $_SESSION['_created']  = time();   // FIX-02: correct session key
    }

    // Clear wizard state — onboarding is done.
    unset($_SESSION['wz']);
...
function handleLogin(): never
{
    ...
    $pdo = Database::getInstance();
    // FIX-03: Use PASSWORD_ALGO constant for rehash check.
    ...
    $_SESSION['user_uuid'] = $user['uuid'];
    $_SESSION['user_role'] = $user['role'];
    // FIX-02: '_created' matches session.php's timeout guard key.
