 ...
 *
 *   BROKER (6 steps):
 *     1  role + email  →  fires OTP  →  stores pending user row
 *     2  OTP verify    →  marks email_verified = 1
 *     3  SalesDesk name (user-chosen slug + display name)
 *     4  profile       →  first_name, last_name, phone, bio
 *     5  password      →  sets password_hash
 *     6  address       →  province → city → suburb (skippable)
 *     99  success + auto-login
 *
 *   DEALER (5 steps):
 *     1  role + email
 *     2  OTP verify
 *     3  company profile → company_name, brand_focus, CIPC upload, account holder name
 *     4  password
 *     5  address (dealership location → dealers.address_id)
 *     99  success + auto-login
 *
 *   SALES EXEC (6 steps):
 *     1  role + email
 *     2  OTP verify
 *     3  dealership selection → creates sales_executives (pending)
 *     4  profile
 *     5  password
 *     6  address (skippable)
 *     99  pending-approval screen (NOT auto-logged in)
 */
...
<!-- ── Progress bar (hidden on step 99) ── -->
<?php if ($step < 99): ?>
...
<p class="skip-link">
  <a href="/auth/authentication.php?action=wizard_skip_dealership">
    My dealership isn't registered yet — skip for now
  </a>
</p>
...
<div class="success-wrap">
  ...
  <?php elseif ($isSalesExec): ?>
    <div class="success-icon"><i class="fa-solid fa-circle-check"></i></div>
    <h1>Account created, <em><?= htmlspecialchars($wz['first_name']) ?></em></h1>
    <p>You can link a dealership from your dashboard when you're ready.</p>
    <a href="/app/exec/dashboard.php" class="btn-auth">Go to dashboard <i class="fa-solid fa-arrow-right"></i></a>

  <?php elseif ($isDealer): ?>
    <div class="success-icon"><i class="fa-solid fa-circle-check"></i></div>
    <h1>You're live, <em><?= htmlspecialchars($wz['company_name'] ?? 'your dealership') ?></em></h1>
    <p>Your dealer account is ready. Head to your dashboard to list your first cars and start receiving leads from our broker network.</p>
    <?php if (!empty($wz['cipc_doc_url'])): ?>
    <div class="alert alert-info" style="text-align:left;margin-bottom:1rem;">
      <i class="fa-solid fa-clock alert-icon"></i>
      Your CIPC document has been submitted for review. You'll be notified once verified.
    </div>
    <?php endif; ?>
    <div class="success-meta">
      <div class="success-meta-row"><span>Role</span><span>Dealer Principal</span></div>
      <div class="success-meta-row"><span>Dealership</span><span><?= htmlspecialchars($wz['company_name'] ?? '') ?></span></div>
      <div class="success-meta-row"><span>Email</span><span><?= htmlspecialchars($wz['email']) ?></span></div>
      <div class="success-meta-row"><span>Account holder</span><span><?= htmlspecialchars(trim(($wz['first_name'] ?? '') . ' ' . ($wz['last_name'] ?? ''))) ?></span></div>
    </div>
    <a href="/app/dealer/dashboard.php" class="btn-auth">Go to my dashboard <i class="fa-solid fa-arrow-right"></i></a>

  <?php else: ?>
    <div class="success-icon"><i class="fa-solid fa-circle-check"></i></div>
    <h1>You're all set, <em><?= htmlspecialchars($wz['first_name']) ?></em></h1>
    <p>Your SalesDesk account is ready. Head to your dashboard to start sharing cars and earning commission.</p>
    <div class="success-meta">
      <div class="success-meta-row"><span>Role</span><span>Auto Broker</span></div>
      <div class="success-meta-row"><span>Desk</span><span><?= htmlspecialchars($wz['desk_display_name'] ?? 'My SalesDesk') ?></span></div>
      <div class="success-meta-row"><span>Email</span><span><?= htmlspecialchars($wz['email']) ?></span></div>
      <div class="success-meta-row"><span>Name</span><span><?= htmlspecialchars(trim($wz['first_name'] . ' ' . $wz['last_name'])) ?></span></div>
    </div>
    <a href="/app/broker/dashboard.php" class="btn-auth">Go to my dashboard <i class="fa-solid fa-arrow-right"></i></a>
  <?php endif; ?>
...
// ── Step-specific JS ──────────────────────────────────────────
// Appended after layout renders so global.js is already loaded.
ob_start();
...
(function () {
  ...
  var lastChecked   = '';
  var lastResult    = null; // true = available, false = taken
  ...
  function onInput() {
    ...
    // Cache hit — reuse result without a new request.
    if (slug === lastChecked && lastResult !== null) {
      applyResult(slug, lastResult);
      return;
    }
  ...
  async function checkSlug(slug) {
    ...
    try {
      ...
      var data = await res.json();
      lastChecked = slug;
      lastResult  = data.available === true;
      // Only apply if the field still shows this slug.
      if (toSlug(input.value) === slug) {
        applyResult(slug, lastResult);
      }
    } catch (e) {
      // API not yet reachable — fail open so wizard isn't blocked.
      setHint('');
      setSubmit(true);
    }
  }
  ...
  function applyResult(slug, available) {
    if (available) {
      setHint(
        '<i class="fa-solid fa-circle-check" style="font-size:10px;color:var(--green)"></i>'
        + ' <span style="color:var(--green)">' + esc('salesdesk.co.za/' + slug) + ' is available</span>',
        'var(--green)'
      );
      setSubmit(true);
    } else {
      setHint(
        '<i class="fa-solid fa-circle-xmark" style="font-size:10px;color:var(--red)"></i>'
        + ' <span style="color:var(--red)">That slug is taken — try a different name</span>',
        'var(--red)'
      );
      setSubmit(false);
    }
  ...
  // On load: if field is pre-filled (wizard resume), run check immediately.
  if (input.value) {
    var slug = toSlug(input.value);
    showPreview(slug);
...
(function restoreSelects() {
  ...
  if (sp) {
    ...
    if (munis.length) {
      populateSelect(document.getElementById('municipality'), munis, sm);
      document.getElementById('muni-group').style.display = 'block';
    }
  }
  if (sm) {
    var cities = (LOC.cities || {})[sm] || [];
    if (cities.length) {
      populateSelect(document.getElementById('city'), cities, sc);
  ...
  if (sc) {
    ...
    if (subs.length) {
      populateSelect(document.getElementById('suburb'), subs, ss);
      document.getElementById('suburb-group').style.display = 'block';
    }
  }
})();
</script>
...
<?php
// Capture extra JS to inject after layout's global.js load.
