 ...
 * T2 owns this file.
 *
 * Three states on this single page:
 *   request  — user enters their email
 *   verify   — 6-digit OTP entry
 *   reset    — set new password (session-token gated)
 *
...
// Logged-in users don't need to reset here.
...
function _handleReset(): never
{
    ...
    $pdo->prepare("UPDATE otp_codes SET used = 1 WHERE user_id = ? AND purpose = 'password_reset' AND used = 0")->execute([$uid]);
    unset($_SESSION['pw_reset']);

    $row = $pdo->prepare("SELECT email FROM users WHERE id = ?")->execute([$uid])
        ? $pdo->query("SELECT email FROM users WHERE id = {$uid}")->fetch()
        : false;
    if ($row) sendPasswordChangedNotice($row['email']);
...
// For reset state — validate session token before rendering form.
$tokenOk = false;
if ($mode === 'reset') {
    $stored   = $_SESSION['pw_reset'] ?? [];
    $tokenOk  = (
