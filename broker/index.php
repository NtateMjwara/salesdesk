<?php
/**
 * SalesDesk — Broker Public Storefront
 * Route: /{broker-slug}/index.php  (or /{broker-slug}/ via .htaccess)
 *
 * Every broker has a personalised public page showcasing their
 * active desk inventory. The slug is salesdesks.slug (set during
 * broker onboarding).
 *
 * Attribution: all car links from this page embed the broker's
 * tracking codes so any lead submitted carries correct attribution.
 *
...
$statsStmt = $pdo->prepare("
    SELECT
        ...
        COUNT(DISTINCT CASE WHEN l.status='closed'
              THEN l.id END)                            AS deals_closed,
...
// ── Org membership (show org badge if broker is in an org) ────
$orgStmt = $pdo->prepare("
...
// ── Wishlist state for current visitor ────────────────────────
$wishlistedIds = [];
if (!empty($inventory)) {
...
// Share URL for the storefront itself.
...
<div style="margin-bottom:1rem;display:flex;align-items:center;justify-content:space-between;
  ...
  <div>
    <span style="font-family:var(--font-d);font-size:16px;font-weight:700;">
      Available cars
    </span>
  ...
  <a href="/c/" style="font-size:12px;color:var(--p);text-decoration:none;">
    Browse all cars →
  </a>
</div>
...
<div style="text-align:center;padding:3rem 1rem;color:var(--faint);
  ...
  <i class="fa-solid fa-car" style="font-size:36px;display:block;
                                     margin-bottom:12px;color:var(--border);"></i>
...
<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(240px,1fr));
            gap:16px;" id="storeInventory">
  <?php foreach ($inventory as $car):
    $imgs    = json_decode($car['image_urls'] ?? '[]', true) ?: [];
    $thumb   = $imgs[0] ?? null;
    $carUrl  = '/c/' . htmlspecialchars($car['car_slug']) . '/?ref='
               . htmlspecialchars($car['tracking_code']);
    ...
    $commDisp = 'R ' . number_format($commRand, 0, '.', ' ');
  ...
  <div class="pub-browse-card pub-reveal" style="position:relative;">
    ...
    <a href="<?= $carUrl ?>" style="text-decoration:none;display:contents;">
      ...
      <div class="pub-browse-card__body">
        ...
        <!-- Commission chip (shows to buyers that a broker is involved) -->
        <div style="display:flex;align-items:center;gap:5px;
                    background:var(--gr-bg);border:1px solid var(--gr-b);
                    border-radius:var(--r-sm);padding:4px 8px;margin-top:4px;">
...
<!-- ── Bio section (below inventory) ────────────────────────── -->
<?php if ($desk['bio']): ?>
<div style="margin-top:2rem;background:var(--white);border:1px solid var(--border);
