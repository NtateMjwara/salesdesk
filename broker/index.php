<?php
/**
 * SalesDesk — Broker Public Storefront  (v3)
 ...
 *   page (/c/index.php), so buyers get one consistent browsing UX
 *   whether they're on a broker's desk or the global catalogue.
 *   Every filter/count/fetch query is scoped to this desk's inventory
 *   only via `bi.salesdesk_id = ?` — nothing here can leak another
 *   desk's cars.
 *
 ...
 *   relevant on the global /c/ page where a car can appear on many
 *   desks). ?ref= is still appended for analytics continuity with
...
// ══════════════════════════════════════════════════════════════
// FILTERS — identical shape to /c/index.php, scoped to this desk
// ══════════════════════════════════════════════════════════════
...
// ── Active-filter count + query string builder (mirrors /c/) ───
$activeFilterCount = 0;
...
/** Fuel icon helper — local copy so this file has no dependency on /c/. */
 ...
 * while preserving everything else. Same null-safe pattern as /c/.
 */
...
<!-- ══════════════════════════════════════════════════════
     INVENTORY — same sidebar filters + vehicle-card grid as /c/
     ══════════════════════════════════════════════════════ -->
