 ...
 *
 * v2.2 fixes (this pass):
 *   FIX-9   Removed redundant <div class="pub-page-wide"> wrapper — layout-public.php
 *           v5 already provides <main class="pub-page-wide">, so the wrapper
 *           was double-nesting and double-padding all content.
 *   FIX-10  Removed browse-grid--animated class from car grid — the CSS animation
 *           starts the grid at opacity:0 (fill-mode:both). If the browser disables
 *           animations (prefers-reduced-motion, background tab, etc.) the inline
 *           <style> block's re-declaration overrides browse.css's
 *           @media prefers-reduced-motion guard, leaving cards permanently invisible.
 *   FIX-11  Moved page-scoped CSS to $extraCss (injected into <head> by
 *           layout-public.php) so it arrives before content renders — eliminates
 *           FOUC and the prefers-reduced-motion override bug. Removed all rules
 *           already present in browse.css; kept only the one genuinely new override
 *           (.browse-layout margin-inline / padding-inline responsive clamp).
 *
...
// Only rules genuinely new to this page; everything else is already
// in browse.css and does not need to be repeated here.
// Critically, the prefers-reduced-motion guard is included so this
// block cannot override browse.css's @media rule regardless of
...
/* Safety: preserve browse.css prefers-reduced-motion guard.
   Without this, a later <style> block would override it. */
...
<?php /* ── Browse page root ─────────────────────────────────────────────
         FIX-9: The <div class="pub-page-wide"> wrapper that previously
         lived here has been removed. layout-public.php v5 already outputs
         <main class="pub-page-wide"> so the wrapper was double-nesting and
         double-padding all browse content. Nothing to wrap here.
         ──────────────────────────────────────────────────────────────── */ ?>
  ...
  <div class="browse-layout">
    ...
    <div class="browse-results">
      ...
      <?php /*
             ...
             FIX-10: Removed class="browse-grid--animated" from this div.
             The animation used animation-fill-mode:both which starts the
             grid at opacity:0. If the browser disables animations
             (prefers-reduced-motion, background tab, etc.) and the inline
             <style> block's re-declaration had overridden browse.css's
             @media guard, the grid would stay permanently invisible.
             Cards are now always visible; browse.css handles the subtle
             fade-up on full-CSS loads without the invisibility risk.
             ─────────────────────────────────────────────────────────── */ ?>
...
<?php /* FIX-11: Inline <style> block removed from here.
         All page-scoped overrides have been moved to $extraCss above
         ob_start() so they are injected into <head> by layout-public.php.
         Rules already present in browse.css were not carried over — only
         the .browse-layout margin-inline / padding-inline clamp is new. */ ?>
