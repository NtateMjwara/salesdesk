...
/**
 * SalesDesk — Careers
 * Route: /careers/  →  careers/index.php
 * .htaccess: resolves via the existing "real directory" pass-through
 * rule (RewriteCond %{REQUEST_FILENAME} -d) — no rewrite rule needed.
 *
 * Openings are read live from job_postings (status = 'published'),
 * managed via app/admin/careers.php + careers-edit.php. This page shows
 * only what's actually in the database — no static/demo fallback data.
 * If the table is missing (migration not run) or the query fails, the
 * page still renders — visitors see the "no open roles" empty state
 * rather than a fatal error, and the real cause is logged for an admin
 * to fix (run db/0009_careers.sql).
 *
 * Filtering (department / location / search) is client-side: the
 * dataset is small and this avoids a request round-trip for a page
 * like this.
 */
...
// ── Live openings — straight from job_postings, no fallback data ──
$openings = [];
try {
    $openings = getPublishedJobPostings();
} catch (Throwable $e) {
    // Table likely doesn't exist yet — run db/0009_careers.sql.
    error_log('[SalesDesk careers] getPublishedJobPostings failed: ' . $e->getMessage());
    $openings = [];
}

$departments = array_values(array_unique(array_column($openings, 'department')));
sort($departments);
$locations = array_values(array_unique(array_column($openings, 'location')));
sort($locations);

// Logged-in admins get a hint instead of guessing why the list is empty;
// regular visitors just see the normal "no roles" empty state below.
...
<section class="cr-benefits">
  <div class="cr-container cr-benefits__grid">
    <div class="cr-benefit"><i class="fa-solid fa-briefcase-medical"></i><span>Medical aid contribution</span></div>
    <div class="cr-benefit"><i class="fa-solid fa-umbrella-beach"></i><span>Uncapped annual leave</span></div>
    <div class="cr-benefit"><i class="fa-solid fa-laptop"></i><span>Home office setup budget</span></div>
    <div class="cr-benefit"><i class="fa-solid fa-graduation-cap"></i><span>Learning &amp; conference budget</span></div>
    <div class="cr-benefit"><i class="fa-solid fa-car-side"></i><span>Staff vehicle discounts</span></div>
  </div>
</section>
...
<section class="cr-section" id="openings">
  <div class="cr-container">
    ...
    <div class="cr-empty">
      ...
      <div class="cr-empty__title">No open roles right now</div>
      <div class="cr-empty__sub">Check back soon, or send us a speculative application below.</div>
      <?php if ($isAdminViewer): ?>
      <div class="cr-empty__admin-hint">
        <i class="fa-solid fa-lock"></i>
        Admin note: no published postings found. Run <code>db/0009_careers.sql</code> if this is a
        fresh install, then add roles from
        <a href="/app/admin/careers-edit.php">the admin panel</a>.
      </div>
      <?php endif; ?>
    </div>
    <?php else: ?>
    <div class="cr-job-list" id="crJobList">
      ...
      <article class="cr-job-card"
                data-dept="<?= htmlspecialchars($job['department']) ?>"
                data-loc="<?= htmlspecialchars($job['location']) ?>"
                data-search="<?= htmlspecialchars(strtolower($job['title'] . ' ' . $job['department'] . ' ' . $job['blurb'])) ?>">
        <div class="cr-job-card__main">
          <div class="cr-job-card__top">
            <h3 class="cr-job-card__title"><?= htmlspecialchars($job['title']) ?></h3>
            <span class="cr-job-card__dept-badge"><?= htmlspecialchars($job['department']) ?></span>
          </div>
        ...
        <a href="/careers/apply/?role=<?= urlencode($job['slug']) ?>" class="cr-job-card__cta">
...
<section class="cr-cta">
  <div class="cr-container cr-cta__inner">
    ...
    <a href="/careers/apply/" class="cr-btn cr-btn-light">
...
.cr-empty__admin-hint i { margin-top: 2px; flex-shrink: 0; }
.cr-empty__admin-hint code {
  font-family: var(--mono);
  background: rgba(0,0,0,.06);
  padding: 1px 5px;
  border-radius: 4px;
}
.cr-empty__admin-hint a { color: var(--amber); text-decoration: underline; }
...
(function () {
  ...
  if (!deptSel || !locSel || !searchInp || !listEl) return;
