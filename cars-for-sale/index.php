...
/**
 * WHITELIST FIX (this pass): these three whitelists previously used
 * values that don't match what app/dealer/car-upload.php's wizard
 * actually writes to cars.fuel_type / cars.transmission / cars.drivetrain
 * — confirmed by reading that file directly. Since these arrays are
 * fixed PHP lists (not pulled from the DB the way $bodyTypes above is),
 * a mismatch here means the corresponding sidebar checkbox can NEVER
 * match a real row: array_intersect() against $_GET only lets through
 * values already in the whitelist, and the SQL IN-clause then searches
 * for that exact whitelist string. Corrected to match the wizard:
 ...
 */
$fuelTypeWhitelist = ['Petrol', 'Diesel', 'Electric', 'Hybrid', 'Plug-in Hybrid (PHEV)', 'Hydrogen', 'LPG (Autogas)', 'CNG (Natural Gas)', 'Flex Fuel (E85/Ethanol)'];
$fuelTypes = array_values(array_intersect(
    array_filter(array_map('trim', (array) ($_GET['fuel_type'] ?? []))),
    $fuelTypeWhitelist
));

$transmissionWhitelist = ['Automatic', 'Manual', 'Semi-Automatic', 'DSG', 'CVT'];
$transmissions = array_values(array_intersect(
    array_filter(array_map('trim', (array) ($_GET['transmission'] ?? []))),
    $transmissionWhitelist
));

$drivetrainWhitelist = ['FWD', 'RWD', 'AWD', '4WD'];
    ...
    <div class="browse-results">
      ...
      <div class="pub-browse-grid" id="carGrid">
        <?php foreach ($cars as $car):
          ...
          $thumb   = $imgs[0] ?? null;
          $detailUrl = '/cars-for-sale/' . htmlspecialchars($car['desk_slug']) . '/'
