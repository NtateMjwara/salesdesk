...
/**
 * Reinstate a suspended dealer with full cascade.
 * Reverse of suspendDealer().
 *
 * Cascade:
 *   - users.status → 'active'
 *   - cars.status  → 'active' for all paused listings owned by this dealer
 *   - commissions  → removes [dealer_suspended] note from dealer_notes
 *   - audit_log    → one entry covering the reinstatement
 *
 * @param int $dealerId  dealers.id
 * @param int $adminId   users.id of the admin performing the action
 * @return array{cars_reinstated: int, commissions_unfrozen: int}
 */
function reinstateDealer(int $dealerId, int $adminId): array
{
    $pdo = Database::getInstance();

    // Fetch dealer — confirm they exist and are currently suspended.
    ...
    if ($dealerRow['status'] !== 'suspended') {
        error_log("[SalesDesk] reinstateDealer: dealer #{$dealerId} is not suspended (status={$dealerRow['status']}).");
        return ['cars_reinstated' => 0, 'commissions_unfrozen' => 0];
    }
    ...
    // 2. Restore all paused listings for this dealer.
    // Mirrors the suspend cascade which paused ALL active listings.
    ...
    // 3. Clear the [dealer_suspended] freeze note from pending commissions.
    // Strip the appended note; NULLIF trims rows that become empty after removal.
 ...
 * Deliberately omits date so a failed payout retried the next day
 * still resolves to the same key and cannot be double-inserted.
 */
