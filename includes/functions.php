 ...
 *   FIX-05: getUsersForAdmin() referenced p.car_limit which does not exist
 *           in the profiles table schema (see 0002_initial_schema.sql).
 *           The column is removed from the SELECT. If per-user car limits
 *           are needed in future, add the column via a migration first.
 *
...
/**
 * FIX-05: Removed p.car_limit from SELECT — that column does not exist in
 * the profiles table. The original query would throw a PDOException
 * ("Unknown column 'p.car_limit'") on every admin user-list page load.
 * Add a migration to create the column before re-adding it here.
 */
