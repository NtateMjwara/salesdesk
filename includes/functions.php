...
function transitionCommissionStatus(
    ...
    try {
        ...
        $isPlatformCommission = $commission['broker_id'] === null;

        $allowed = COMMISSION_TRANSITIONS[$currentStatus] ?? [];

        // MIGRATION 0012: a platform commission (no broker attributed)
        // has nobody to pay out to, so it never goes through the
        // scheduled/processing payout pipeline. From 'approved' it can
        // go straight to 'paid' ("dealer invoice settled", not "broker
        // paid"), and — even though COMMISSION_TRANSITIONS allows
        // approved → scheduled for the normal broker case — that path
        // is explicitly removed here, so a platform commission can
        // never reach 'scheduled' and therefore createPayoutRecord()
        // can never be called for it (see that function's docblock).
        if ($isPlatformCommission && $currentStatus === 'approved') {
            $allowed = array_diff($allowed, ['scheduled']);
            $allowed[] = 'paid';
        }
...
/**
 * MIGRATION 0012: c.broker_id is now nullable (platform-attributed
 * commissions have no broker). bu/bp joins changed from JOIN to
 * LEFT JOIN so a platform commission still returns its car/dealer
 * data instead of vanishing from the result set entirely — callers
 * (invoice emails, admin commission views) should treat a null
 * broker_email/broker_first as "platform, no broker to pay" and
 * branch their display accordingly.
 */
...
/**
 * MIGRATION 0012: callers must not invoke this at all for a platform
 * commission (broker_id IS NULL on the commissions row) — there is no
 * broker and no bank account to pay out to. This function's signature
 * stays frozen (still requires a real int $brokerId) precisely so a
 * platform commission literally cannot reach this call without the
 * caller first resolving a broker id, which will fail loudly instead
 * of silently paying out to nobody. transitionCommissionStatus()
 * above now enforces this from the other direction too: a platform
 * commission can never reach 'scheduled' status in the first place,
 * so the codepath that calls createPayoutRecord() on a scheduling
 * transition can never fire for one.
 */
