...
class ImageRehostService
{
    ...
    public function rehost(
        ...
        ?string $referer = null,
        bool $fallbackToSourceUrl = false
    ): ?array
    {
        ...
        if (empty($localUrls)) {
            ...
            if ($fallbackToSourceUrl && !$stoppedEarly) {
                // LAST-RESORT FALLBACK — opt-in only, off by default. Used
                // by MotusApiImporter specifically, after Referer and a
                // real browser User-Agent were both confirmed (against
                // real image URLs, from a real server) to still get
                // blocked. Storing nothing when we have a perfectly good
                // URL sitting right there in the source data is strictly
                // worse than storing that URL directly — so as a genuine
                // last resort, do that instead.
                //
                // READ BEFORE ENABLING THIS FOR ANOTHER IMPORTER:
                //   - These are raw third-party URLs, not re-hosted ones.
                //     They can go dead if the source removes the listing,
                //     and this codebase has no control over their format,
                //     size, or availability.
                //   - They may not even render for real site visitors —
                //     if the source blocks OUR server on Referer/hotlink
                //     grounds, a real visitor's browser sending OUR
                //     domain as Referer could hit the exact same block.
                //     This is a "maybe something is better than nothing"
                //     bet, not a guarantee.
                //   - source_image_urls is deliberately returned EMPTY
                //     here, not the attempted list — so the "unchanged
                //     since last run" short-circuit at the top of this
                //     method never matches, and every future sync() keeps
                //     retrying REAL re-hosting from scratch. If the block
                //     is ever lifted (or a proper fix found), re-hosting
                //     takes over automatically on the next run with no
                //     special handling needed — this fallback doesn't
                //     "stick" once it's been used.
                error_log("[ImageRehostService] {$reason} for car {$carUuid} — falling back to direct source URL(s) rather than storing nothing. This is a last resort, not a fix; re-hosting will keep being retried on future syncs.");
                return [
    ...
    private function downloadOne(string $url, string $dir, string $carUuid, int $index, ?string $referer = null): string
    {
        ...
        $curlOpts = [
            ...
            // A realistic browser UA, not our own self-identifying one —
            // deliberately different from the UA used for page fetches
            // elsewhere in this codebase. CDNs commonly apply much
            // stricter bot-filtering to image/asset paths specifically
            // than to their HTML pages (which sites often want crawlable
            // for SEO), so a bot-identifying UA here is a plausible,
            // simple cause of a 403 that a page fetch with the same
            // "SalesDeskImporter/1.0" string wouldn't hit at all.
