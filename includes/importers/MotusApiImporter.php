...
class MotusApiImporter implements InventoryImporterInterface
{
     ...
     * the page.
     *
     * CONFIRMED against a real production payload (810 vehicles across
     * 10 dealerships) — this is no longer speculative. The real syntax
     * is NOT strict JSON:
     *   - Every value is double-quoted EXCEPT `mileagePretty`, which is
     *     single-quoted (e.g. `'24,291 km'`) — the only field observed
     *     using single quotes anywhere in the payload.
     *   - `model` always appears as a literal, redundant JS OR:
     *     `"AMAROK" || "AMAROK"` — both sides always identical in every
     *     vehicle checked, i.e. dead template code, not real fallback
     *     logic.
     *   - `variant` uses a REAL fallback: `"2.0TDI Single Cab" || ""` —
     *     unlike `model`, the two sides can differ, so this can't be
     *     collapsed the same trivial way; must resolve to whichever side
     *     is non-empty.
     *   - No other field was found using the `||` pattern.
     *   - `stockId` is ALWAYS in scientific-notation string form (all
     *     810 of them, zero exceptions) — e.g. `"1.300168e+06"` for what
     *     is really the plain integer 1300168 — see normalizeStockId().
    ...
    /**
     * Repairs the specific, confirmed JS-literal quirks in VLPData's raw
     * text into valid JSON. Order matters: the OR-expression repair must
     * run before json_decode ever sees the string, since `"X" || "Y"` as
     * a bare value is not valid JSON under any circumstance.
     */
    private function repairVlpDataLiteral(string $raw): string
    {
        // 1. Single-quoted string values (confirmed: mileagePretty only,
        //    but written generically in case another field starts using
        //    it) -> double-quoted. Handles \' and \\ escapes inside.
        $repaired = preg_replace_callback(
            '/:\s*\'((?:\\\\.|[^\'\\\\])*)\'/',
            static function (array $m): string {
                // Un-escape the JS single-quote escaping, then re-escape
                // for JSON double-quote context.
        ...
        // 2. "X" || "Y" fallback expressions -> whichever side is
        //    non-empty (real JS truthy-OR semantics for strings: an
        //    empty string is falsy, so "" || "Y" resolves to "Y"; any
        //    non-empty "X" || "Y" resolves to "X" — matches both the
        //    dead-code `model` case, where both sides are always
        //    identical, and the real `variant` fallback case).
        $repaired = preg_replace_callback(
            '/"((?:\\\\.|[^"\\\\])*)"\s*\|\|\s*"((?:\\\\.|[^"\\\\])*)"/',
            static function (array $m): string {
                $chosen = $m[1] !== '' ? $m[1] : $m[2];
        ...
        // 3. Trailing commas before ] or } — kept as a defensive
        //    fallback; not observed as necessary against the confirmed
        //    real payload, but harmless if absent and cheap insurance
    ...
    /**
     * "1.300168e+06" -> "1300168". Every stockId in the confirmed real
     * payload is a scientific-notation string like this — PHP's float
     * cast parses that notation correctly on its own, so this is just
     * about getting back to a plain integer string for use as a dedup
     * key / display value instead of the scientific-notation form.
     * Falls through to the original string unchanged if it doesn't match
     * that pattern, so a future payload using a plain numeric stockId
     * (or a non-numeric one) isn't corrupted by this normalization.
     */
    ...
    /**
     * Filters VLPData rows down to just the vehicles belonging to one
     * named dealer/branch. Case-insensitive exact match against
     * `dealerName`, since that's the only per-vehicle dealer identifier
     * VLPData carries — there's no numeric dealerId field in the payload
     * itself (the numeric ID visible in relPermalink, e.g. "625" for
     * Midrand, is a MORE reliable signal, but requires the caller to
     * already know it; matching on the human-readable branch name they
     * can see on their own site is the friendlier default).
     *
    ...
    public function sync(int $dealerId, string $sourceRef, array $options = []): array
    {
        ...
        // CONFIRMED against a real payload: VLPData on a Motus showroom
        // listing page is NOT scoped to one branch — one real sample
        // contained 810 vehicles spanning 10 different dealerships
        // (Alberton, Bloemfontein, East Rand Mall, Edenvale, Fourways,
        // Germiston, Kempton Park, Midrand, Vereeniging, Welkom), all in
        // the SAME array regardless of which branch's URL was fetched.
        // Without this filter, sync() would import every competing
        // branch's stock under the calling dealer's account — this is
        // a hard requirement, not an optional refinement, so it's
        // checked and fails fast BEFORE any network request is made.
        if ($dealerNameFilter === '') {
            throw new InvalidArgumentException(
        ...
        foreach ($rows as $i => $rawRow) {
            try {
                ...
                // Referer for this vehicle's dealer site — needed for
                // EVERY image download (not just the gallery-enrichment
                // fetch below), since stock-images.cdn.motus.cars returns
                // HTTP 403 with no Referer at all (confirmed against a
                // real run: images silently failed to re-host even though
                // the import itself succeeded). This is a one-time,
                // server-side header on our own download request — it
                // has no bearing on how the re-hosted image is served to
                // real site visitors afterward, who load it from our own
                // domain with no Referer question at all.
                $referer = ((string) ($rawRow['_base_url'] ?? '')) . '/';

                // VLPData itself only ever has ONE cover photo per
                // vehicle (confirmed against a real payload — no other
                // image field exists there). The full gallery only lives
                // on the vehicle's own detail page, so fetch it there —
                // but only if there's time left in the budget; if not,
                // fall back to the single cover photo already set by
                // parseVehicle() rather than block this vehicle entirely.
                // A future sync() run will pick up the fuller gallery for
                // whatever didn't get reached this time, same "stop early,
                // retry next run" pattern used everywhere else in this
                // codebase.
                if (microtime(true) < $deadline && $vehicle['source_url'] !== '') {
                    $detailHtml = $this->fetchPage($vehicle['source_url']);
                    if ($detailHtml !== null) {
                        $gallery = MotusShowroomParser::extractImagesFromHtml($detailHtml);
                        if (!empty($gallery)) {
                            $vehicle['image_urls'] = $gallery;
                        }
                    }
                }
                ...
                if ($existing !== null) {
                    $this->updateCar($existing, $dealerId, $vehicle, $commType, $commValue, $importId, $deadline, $referer);
                    $counts['updated']++;
                } else {
                    $this->insertCar($dealerId, $vehicle, $commType, $commValue, $importId, $execId, $deadline, $referer);
                    $counts['imported']++;
                }
            } catch (Throwable $e) {
                $counts['failed']++;
    ...
    private function insertCar(int $dealerId, array $v, string $commType, float $commValue, int $importId, ?int $execId, ?float $deadlineTs = null, ?string $referer = null): void
    {
        $uuid   = generateUuidV4();
        $slug   = $this->generateUniqueSlug($dealerId, $v['year'], $v['make'], $v['model'], $v['colour']);
        $images = $this->rehostImages($uuid, $v['image_urls'], [], $deadlineTs, $referer);
    ...
    private function updateCar(array $existing, int $dealerId, array $v, string $commType, float $commValue, int $importId, ?float $deadlineTs = null, ?string $referer = null): void
    {
        $carId = $existing['id'];
        $uuid  = $existing['uuid'];

        $images       = $this->rehostImages($uuid, $v['image_urls'], $existing['source_image_urls'], $deadlineTs, $referer);
