...
class MotusShowroomParser
{
    ...
    private const TOP_LEVEL_LABELS = [
        ...
        'fuel type'    => 'fuelType',
        'stock number' => 'sku',
        // Confirmed on real pages: a "Dealership" heading (no colon) whose
        // value is the branch name, e.g. "Motus VW Midrand" — sits under
        // "Dealership Information". Needed so MotusApiImporter can verify
        // a fetched detail page actually belongs to the branch it was
        // supposed to (see detailPageMatchesVehicle()'s docblock for why).
    ...
    public function extract(string $html): array
    {
        ...
        // Tracks whether the PREVIOUS heading in document order was the H1
        // hero title, so the very next heading can be checked for the
        // free-text variant/trim line — see the capture logic below.
        $prevWasHero    = false;

        foreach ($headingNodes as $el) {
            $rawText = trim(preg_replace('/\s+/', ' ', $el->textContent ?? ''));
            $label   = $this->normalizeLabel($el->textContent ?? '');
            $isHero  = strtolower($el->nodeName) === 'h1';

            // CONFIRMED against real production markup: every page seen so
            // far has a free-text trim/variant line (e.g. "Hatch 1.0TSI
            // Life Manual") as the heading immediately following the H1
            // hero title (e.g. "2025 VOLKSWAGEN POLO") — with no "Label:"
            // prefix of its own, so it would otherwise be silently skipped
            // by the label-driven logic below (nextValueText() finds
            // nothing useful after it, and it never matches
            // TOP_LEVEL_LABELS since it isn't one). Captured here, using
            // the ORIGINAL text (not normalizeLabel()'s lowercased,
            // colon-stripped form), since this is itself a value, not a
            // label. Guarded by isKnownLabel() so a page that happens to
            // jump straight from H1 to a real "Model:" heading (no variant
            // line at all) doesn't wrongly capture "Model:" as the variant.
            if ($prevWasHero && !$isHero && $rawText !== '' && !$this->isKnownLabel($label)) {
                if (!isset($node['variant'])) {
                    $node['variant'] = $rawText;
                }
            }
            $prevWasHero = $isHero;
