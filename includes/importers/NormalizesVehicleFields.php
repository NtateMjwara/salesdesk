...
trait NormalizesVehicleFields
{
    ...
    private function parseNumber(string $raw): ?float
    {
        ...
        // CORRECTED: confirmed via a real Motus production page that
        // rendered amounts can use a non-breaking space (U+00A0, UTF-8
        // 0xC2 0xA0) as a separator (e.g. "R\u{00A0}329,995") rather than
        // a plain ASCII space. The str_replace(' ', ...) below only ever
        // stripped the single-byte ASCII space, leaving the NBSP bytes
        // in place and breaking the anchored digit regex further down —
        // normalizing NBSP to a regular space first fixes that for every
        // caller of this shared helper, not just the one site it was
        // first noticed on.
