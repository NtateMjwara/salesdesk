...
class WebsiteImporter implements InventoryImporterInterface
{
    ...
    public function crawlDealership(string $sourceRef): array
    {
        ...
        $candidates = $this->discoverCandidateUrls($base, $host);
        ...
        foreach ($candidates as $url) {
            ...
            foreach ($this->parser->extract($html) as $node) {
     ...
     * @return string[]
     */
    private function discoverCandidateUrls(string $base, string $host): array
    {
        $fromSitemap = $this->urlsFromSitemap($base);
    ...
    /** @return string[] */
    private function urlsFromSitemap(string $base): array
    {
        foreach (['/sitemap.xml', '/sitemap_index.xml'] as $path) {
            ...
            $urls = $this->parseSitemapXml($xml, $base);
            if (!empty($urls)) {
                return $urls;
            }
        }
    ...
    /** @return string[] */
    private function parseSitemapXml(string $xml, string $base): array
    {
        ...
        if (isset($doc->sitemap)) {
            foreach ($doc->sitemap as $entry) {
                ...
                if ($childXml !== null) {
                    $urls = array_merge($urls, $this->parseSitemapXml($childXml, $base));
                }
