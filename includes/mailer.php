 ...
 *
 *   LEAD NOTIFICATIONS — supply side  (D-03 follow-up)
 *     sendNewLeadEmail(to, recipientName, roleLabel, lead)
 *     sendDealClosedEmail(to, recipientName, roleLabel, context)
 *
 *   COMMISSIONS  (D-08)
 *     sendDealerCommissionInvoice(dealer, lead, commission)
 *
 *   PAYOUTS  (p2, p3)
 *     sendPayoutPaidEmail(commission, referenceNumber)
...
// LEAD NOTIFICATIONS — supply side  (D-03 follow-up)
//
// Rationale: sendBuyerConfirmation() covers the buyer. Until now,
// the broker, dealer principal, and (where relevant) the uploading
// sales exec only received in-app notifications rows — no email.
// These two functions close that gap.
//
// Both functions are intentionally role-generic (one function,
// three possible callers) rather than three near-duplicate
// functions, since the underlying content is identical and only
// the greeting / dashboard link differ per role.
//
// Contract: callers pass $roleLabel as one of 'broker' | 'dealer' |
// 'sales_exec'. Any other value falls back to the broker dashboard
// link but otherwise still sends — fail-soft, not fail-closed,
// to avoid silently dropping a notification on an unexpected value.
// ============================================================

/**
 * Notify a broker, dealer principal, or sales exec that a new lead
 * came in on a car they're connected to. Same content for all three
 * roles — only the greeting and the dashboard link differ.
 *
 * @param string $to            Recipient email address
 * @param string $recipientName Display name for the greeting (falls back to "there")
 * @param string $roleLabel     'broker' | 'dealer' | 'sales_exec' | 'platform' (MIGRATION 0012)
 * @param array  $lead          Must have: buyer_name, buyer_phone, buyer_intent,
...
function sendNewLeadEmail(
    ...
    $name     = htmlspecialchars($recipientName ?: 'there');
    $carLabel = htmlspecialchars(
        trim(
            ($lead['car_year']  ?? '') . ' ' .
            ($lead['car_make']  ?? '') . ' ' .
    ...
    $buyerPhone = htmlspecialchars($lead['buyer_phone'] ?? '—');
    $intentCopy = match ($lead['buyer_intent'] ?? 'browsing') {
        'within_30d' => '🔥 Looking to buy within 30 days',
        'one_to_3mo' => '🗓️ Looking to buy in 1–3 months',
        default      => '👀 Just browsing for now',
    };
    $deskLine = !empty($lead['desk_name'])
        ? '<p style="font-size:13px;color:#64748b;margin:0 0 16px;">Via <strong>'
          . htmlspecialchars($lead['desk_name']) . '</strong></p>'
        : '';

    // MIGRATION 0012: platform-attributed lead (no broker). Called
    // from api/leads/submit.php with $to = 'sales@salesdesk.co.za',
    // $recipientName = 'SalesDesk Team'. No confirmed internal
    // admin lead-review URL exists in the reviewed codebase, so the
    // CTA button is omitted entirely rather than link to a guessed,
    // possibly-nonexistent route — update $dashboardUrl below once
    // that route is confirmed.
    $platformNote = $roleLabel === 'platform'
        ? '<p style="font-size:13px;color:#94a3b8;line-height:1.6;margin:0 0 16px;">'
          . 'This lead has no broker attribution — it came in via the platform browse page.</p>'
        : '';

    $dashboardUrl = match ($roleLabel) {
        'sales_exec' => SITE_URL . '/app/exec/leads.php',
        'dealer'     => SITE_URL . '/app/dealer/leads.php',
        'platform'   => null,
        default      => SITE_URL . '/app/broker/leads.php', // 'broker' and any unexpected value
    };
    $ctaBlock = $dashboardUrl
        ? <<<HTML
<a href="{$dashboardUrl}" style="display:inline-block;background:#0f4c9e;color:#fff;
   font-size:15px;font-weight:600;padding:12px 26px;border-radius:8px;text-decoration:none;">
  View lead →
</a>
HTML
    ...
    $heading = $roleLabel === 'platform' ? 'New lead — no broker attribution' : "New lead: {$carLabel}";
    $subjectPrefix = $roleLabel === 'platform' ? 'New platform lead' : 'New lead';

    $body = <<<HTML
<h2 style="font-size:20px;font-weight:700;color:#0f4c9e;margin:0 0 8px;">
  {$heading}
</h2>
<p style="font-size:15px;color:#475569;line-height:1.65;margin:0 0 4px;">
  Hi {$name},
</p>
{$deskLine}
{$platformNote}
<div style="background:#f8faff;border:1px solid #e2e8f0;border-radius:10px;
  ...
  <p style="font-size:13px;color:#64748b;margin:0 0 4px;">Buyer</p>
  <p style="font-size:15px;font-weight:600;color:#1e293b;margin:0 0 10px;">{$buyerName}</p>
  <p style="font-size:13px;color:#64748b;margin:0 0 4px;">Phone</p>
  <p style="font-size:15px;font-weight:600;color:#1e293b;font-family:monospace;margin:0 0 10px;">{$buyerPhone}</p>
  <p style="font-size:13px;color:#475569;margin:0;">{$intentCopy}</p>
</div>
{$ctaBlock}
HTML;

    return sendEmail($to, "{$subjectPrefix} — {$carLabel}", $body);
}

/**
 * Notify a broker or sales exec that a deal closed on their lead.
 * Deliberately lighter-weight than sendDealerCommissionInvoice() —
 * no banking details here, since neither role is the invoice payee.
 *
 * @param string $to            Recipient email address
 * @param string $recipientName Display name for the greeting
 * @param string $roleLabel     'broker' | 'sales_exec'
 * @param array  $context       Must have: buyer_name, car_make, car_model, car_year
 *                               Optional: net_amount — shown only when roleLabel === 'broker',
 *                               since the exec doesn't receive the commission directly.
 */
function sendDealClosedEmail(
    string $to,
    string $recipientName,
    string $roleLabel,
    array $context
    ...
    $buyerName = htmlspecialchars($context['buyer_name'] ?? '—');

    $earningsBlock = '';
    if ($roleLabel === 'broker' && isset($context['net_amount'])) {
        $net = formatZAR((float) $context['net_amount']);
        $earningsBlock = <<<HTML
<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;
            padding:14px 18px;margin:0 0 16px;">
  <p style="font-size:13px;color:#166534;margin:0;">
    Commission of <strong>{$net}</strong> has been created and is pending approval.
  </p>
</div>
HTML;
    }

    $dashboardUrl = $roleLabel === 'sales_exec'
        ? SITE_URL . '/app/exec/leads.php'
        : SITE_URL . '/app/broker/earnings.php';
...
<p style="font-size:15px;color:#475569;line-height:1.65;margin:0 0 16px;">
  The deal on the <strong>{$carLabel}</strong> with <strong>{$buyerName}</strong> has been marked closed.
</p>
{$earningsBlock}
<a href="{$dashboardUrl}" style="display:inline-block;background:#0f4c9e;color:#fff;
   font-size:15px;font-weight:600;padding:12px 26px;border-radius:8px;text-decoration:none;">
...
HTML;
    ...
    return sendEmail($to, "Deal closed — {$carLabel}", $body);
}
...
function sendDealerCommissionInvoice(
    ...
    // EFT banking details — read from platform_config so they can be
    // updated without a code deploy. Falls back to placeholder if not set.
    $bankName    = htmlspecialchars(getPlatformConfig('salesdesk_bank_name',    'FNB'));
    $bankAccount = htmlspecialchars(getPlatformConfig('salesdesk_bank_account', '62000000000'));
    $bankBranch  = htmlspecialchars(getPlatformConfig('salesdesk_bank_branch',  '250655'));
    $bankHolder  = htmlspecialchars(getPlatformConfig('salesdesk_bank_holder',  'SalesDesk (Pty) Ltd'));
...
// ============================================================
// PAYOUT EMAIL HELPERS  (p2, p3)
// ============================================================

/**
 * Notify a broker that their payout has been processed.
 *
 * @param array  $commission  Full commission row from getCommissionById()
 * @param string $reference   EFT reference number entered by admin
 */
function sendPayoutPaidEmail(array $commission, string $reference): bool
{
    $to        = $commission['broker_email'] ?? '';
    if (!$to || !filter_var($to, FILTER_VALIDATE_EMAIL)) return false;

    $brokerName = htmlspecialchars(
        trim(($commission['broker_first'] ?? '') . ' ' . ($commission['broker_last'] ?? ''))
        ?: 'there'
    );
    $carLabel   = htmlspecialchars(
        ($commission['car_year']  ?? '') . ' ' .
        ($commission['car_make']  ?? '') . ' ' .
        ($commission['car_model'] ?? '')
    );
    $gross      = formatZAR((float) ($commission['gross_amount'] ?? 0));
    $fee        = formatZAR((float) ($commission['platform_fee'] ?? 0));
    $net        = formatZAR((float) ($commission['net_amount']   ?? 0));
    $ref        = htmlspecialchars($reference);
    $earningsUrl = SITE_URL . '/app/broker/earnings.php';
...
<p style="font-size:15px;color:#475569;line-height:1.65;margin:0 0 20px;">
  Your commission for the <strong>{$carLabel}</strong> has been paid via EFT.
</p>
...
<table cellpadding="0" cellspacing="0" width="100%"
       style="border-collapse:collapse;background:#f8faff;border:1px solid #e2e8f0;
              border-radius:10px;overflow:hidden;margin:0 0 20px;">
  <tr style="background:#eff4ff;border-bottom:1px solid #bfdbfe;">
    <td colspan="2" style="padding:10px 16px;font-size:11px;font-weight:700;
                            letter-spacing:.06em;text-transform:uppercase;color:#64748b;">
      Payout summary
    </td>
  </tr>
  <tr style="border-bottom:1px solid #e2e8f0;">
    <td style="padding:10px 16px;font-size:13px;color:#475569;">Gross commission</td>
    <td style="padding:10px 16px;font-size:13px;font-family:monospace;text-align:right;color:#1e293b;">{$gross}</td>
  </tr>
  <tr style="border-bottom:1px solid #e2e8f0;">
    <td style="padding:10px 16px;font-size:13px;color:#475569;">SalesDesk platform fee</td>
    <td style="padding:10px 16px;font-size:13px;font-family:monospace;text-align:right;color:#dc2626;">&minus; {$fee}</td>
  </tr>
  <tr style="background:#f0fdf4;border-bottom:1px solid #bbf7d0;">
    <td style="padding:12px 16px;font-size:14px;font-weight:700;color:#1e293b;">Net paid to you</td>
    <td style="padding:12px 16px;font-size:18px;font-family:monospace;font-weight:800;
               text-align:right;color:#15803d;">{$net}</td>
  </tr>
  <tr>
    <td style="padding:10px 16px;font-size:13px;color:#475569;">EFT reference</td>
    <td style="padding:10px 16px;font-size:13px;font-family:monospace;text-align:right;color:#1e293b;">{$ref}</td>
  </tr>
</table>

<p style="font-size:13px;color:#64748b;line-height:1.6;margin:0 0 18px;">
  Please allow 1–2 business days for the EFT to clear in your account. Contact us at
  <a href="mailto:accounts@salesdesk.co.za" style="color:#0f4c9e;">accounts@salesdesk.co.za</a>
  if the payment hasn't arrived after 3 business days.
</p>

<a href="{$earningsUrl}"
   style="display:inline-block;background:#0f4c9e;color:#fff;font-size:15px;
          font-weight:600;padding:12px 26px;border-radius:8px;text-decoration:none;">
  View earnings →
</a>
HTML;

    return sendEmail($to, "Commission paid — {$net} for {$carLabel}", $body);
}


/**
 * Notify a broker that their payout failed and ask them to check bank details.
 *
 * @param array $commission  Full commission row from getCommissionById()
 */
function sendPayoutFailedEmail(array $commission): bool
{
    $to        = $commission['broker_email'] ?? '';
    if (!$to || !filter_var($to, FILTER_VALIDATE_EMAIL)) return false;
    ...
    $body = <<<HTML
<h2 style="font-size:20px;font-weight:700;color:#dc2626;margin:0 0 8px;">
  Action required — payout unsuccessful
</h2>
<p style="font-size:15px;color:#475569;line-height:1.65;margin:0 0 8px;">
  Hi {$brokerName},
</p>
<p style="font-size:15px;color:#475569;line-height:1.65;margin:0 0 20px;">
  We were unable to process your commission payout of <strong>{$net}</strong> for the
  <strong>{$carLabel}</strong>.
</p>
<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:10px;
            padding:14px 18px;margin:0 0 20px;">
  <p style="font-size:13px;color:#7f1d1d;margin:0;">
    <strong>What to do:</strong> Please verify your banking details in your SalesDesk
    account. Once confirmed, our team will retry the payment within 1 business day.
  </p>
</div>
<a href="{$earningsUrl}"
   style="display:inline-block;background:#0f4c9e;color:#fff;font-size:15px;
...
<p style="font-size:13px;color:#94a3b8;margin:18px 0 0;">
  Questions? Email <a href="mailto:accounts@salesdesk.co.za" style="color:#0f4c9e;">accounts@salesdesk.co.za</a>
  and quote your lead reference.
</p>
HTML;

    return sendEmail($to, "Action required — commission payout failed for {$carLabel}", $body);
}
