...
// FIX: bumped LIMIT 3 → LIMIT 8 to fill the new 4-column x 2-row
// "Browse by category" grid (see home.css .sd-vehicle-grid).
try {
...
// Powers the "Recently Viewed" activity tab. Pulled from car_views,
// joined back to cars + first broker desk so we can build a /c/ URL.
try {
...
// ── Live: Wishlist ───────────────────────────────────────────────
// Powers the "Wishlist" activity tab. getWishlistCarIds() comes from
// includes/visitor.php (visitor_wishlist table).
try {
    ...
    if ($wishlistIds) {
        ...
        // Preserve wishlist order (most-recently-added first) since the
        // IN (...) query above does not guarantee row order.
...
// ── Live: Saved Searches ────────────────────────────────────────
// Powers the "Saved Searches" activity tab.
// Requires db/0008_saved_searches.sql to have been run.
try {
    ...
    // migration — fail gracefully to the empty state.
    $savedSearches = [];
}
 ...
 * Mirrors the markup of .sd-vcard used in "Browse by category" so the
 * same home.css rules apply.
 */
...
// ── Page meta ──────────────────────────────────────────────────
$pageTitle     = '' . number_format($totalCars) . ' New & Used Cars for Sale in South Africa | SalesDesk';
$ogTitle       = 'Search ' . number_format($totalCars) . ' New & Used Cars Across South Africa | SalesDesk';
...
$shareTitle     = 'New & Used Cars for Sale in South Africa | SalesDesk';
...
<!-- ════════════════════════════════════════════════
     ...
     Now hooked to the backend:
       - Recently Viewed  → car_views (per visitor session)
       - Wishlist         → visitor_wishlist
       - Saved Searches   → saved_searches (db/0008_saved_searches.sql)
     ════════════════════════════════════════════════ -->
...
<!-- ════════════════════════════════════════════════
     BROWSE BY CATEGORY
     UI FIX: grid is now centred, 4 columns x 2 rows on
     desktop (8 cars fetched above). On mobile the grid
     stays at 4 columns and scrolls horizontally; the
     quick-nav pill row also scrolls horizontally.
     See home.css .sd-vehicle-grid / .sd-pill-row.
     ════════════════════════════════════════════════ -->
<section class="sd-section">
  <div class="container">
    <div class="sd-sec-header">
    ...
    <!-- Live featured vehicle cards — 4 cols x 2 rows on desktop -->
...
<!-- ════════════════════════════════════════════════
     BROWSE BY PROVINCE
     UI FIX: tiles enlarged + squared (rounded corners
     instead of full pill) on desktop. See home.css
     .sd-prov-grid / .sd-prov-chip.
     ════════════════════════════════════════════════ -->
<section class="sd-section">
  <div class="container">
    <div class="sd-sec-header">
...
<style>
  ...
  .sd-bcard {
    min-width: 0; /* don't let long unbreakable content (e.g. an email name) widen the grid track */
  }
 
  .sd-bcard__name  {
    overflow-wrap: anywhere; /* let emails / long unbroken strings wrap instead of overflowing */
  }
