 ...
 *
 * LOADING BEHAVIOUR:
 *   Auth pages use a button-level spinner rather than the full-screen
 *   page-transition overlay used on public/app pages. When any form
 *   on the page is submitted, initAuthFormLoading() (in global.js):
 *     - Disables the submit button immediately (prevents double-submit)
 *     - Replaces the button label with a spinner + "Submitting…"
 *     - Restores the button after 10 s if navigation never happens
 *       (e.g. client-side validation blocked the submit)
 *   No full-screen overlay is shown — the card itself gives enough
 *   visual context that something is happening.
 */
...
<!-- Global JS (CSRF + OTP widget + nav overlay + auth button loading) -->
...
<script>
  /*
   * Wire auth button loading state.
   *
   * initAuthFormLoading() is defined in global.js. It intercepts all
   * form submits on this page and:
   *   1. Immediately disables the submit button (blocks double-submit)
   *   2. Replaces button content with a spinner + "Submitting…"
   *   3. Restores after 10 s if the page never navigates away
   *      (client-side validation blocked the submit)
   *
   * The full-screen page-transition overlay (initNavOverlay in global.js)
   * is also active on this page for any <a> link clicks. Form submits
   * are deliberately excluded from the overlay — the button spinner
   * provides sufficient feedback within the card context.
   */
  document.addEventListener('DOMContentLoaded', function () {
    if (typeof initAuthFormLoading === 'function') {
      initAuthFormLoading();
    }
  });
</script>
