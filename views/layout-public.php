<?php
/**
 * SalesDesk — Public Layout Shell (v4)
 * T1 owns this file.
 *
 * Changes from v3:
 *   - Footer replaced with full 4-column dark footer:
 *       Col 1: brand + newsletter sign-up + social links
 *       Col 2: Platform links
 *       Col 3: For Brokers links
 *       Col 4: Legal links
 *     Newsletter wiring (POST /api/newsletter/subscribe) deferred to
 *     next session — the form is present but submits to #.
 *   - Footer CSS moved into inline <style> block in this file
 *     (will migrate to public.css in a dedicated CSS pass).
 *
 * Everything else (nav, breadcrumb, share sheet, scripts) is unchanged
 * from v3. See git history for prior layout versions.
 */
...
<head>
  ...
  <!--
    CSS load order (T1 rule — no team may alter this sequence):
      ...
      5. extraCss         — caller-injected page-specific sheet (if any)
  -->
...
<!-- ══════════════════════════════════════════════════════
     MAIN CONTENT
     ══════════════════════════════════════════════════════ -->
<main class="<?= $layoutVariant === 'narrow' ? 'pub-page-narrow' : 'pub-main-wide' ?>"
      id="main-content">
  <?= $pageContent ?>
</main>
