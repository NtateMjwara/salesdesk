...
.hws-filter-btn i { font-size: 13px; opacity: .7; }
...
/* Fuller, centered pills for New/Demo/Used specifically — the
   advanced body/fuel chips further down share .hws-chip but stay
   compact and left-aligned in a wrapping row, so this is scoped
   to .hws-cond-row rather than changing the shared base class. */
.hws-cond-row .hws-chip {
  flex: 1;
  justify-content: center;
  text-align: center;
  padding: 10px 14px;
...
@media (max-width: 480px) {
  ...
  .hws-submit { height: 50px; font-size: 12px; }
...
@media (max-width: 360px) {
  ...
  .hws-filter-btn { flex: none; width: calc(50% - 3px); }
}
</style>
...
<div id="heroSearchWidget">
  ...
  <div class="hws-card">
    ...
    <button class="hws-submit" id="hSubmit" type="button" aria-label="Search cars">
      <span class="h-submit-label">Search <?= $totalCars > 0 ? number_format($totalCars) . ' cars' : 'all cars' ?></span>
    </button>
...
function renderAc(open) {
  ...
  acBox.querySelectorAll('.hac-recent').forEach(function(el){
    el.addEventListener('mousedown', function(e){
      ...
      if (rec) window.location.href = '/cars-for-sale/?' + rec.qs;
    });
  });
...
var fetchCount = debounce(function() {
  ...
  qs.set('per_page','1');
  // MIGRATION 0012: /cars-for-sale/ now shows every active car
  // regardless of desk attribution, so the on_desk_only gate that
  // BUG-SEARCH-01 added here is no longer needed — the default
  // (ungated) count from api/cars/search.php matches what
  // /cars-for-sale/ actually renders again. api/cars/search.php
  // itself is UNCHANGED: the broker marketplace still passes
  // on_desk_only=1 explicitly when it wants the opposite (cars not
  // yet on any desk, for discovery).
...
function doSearch() {
  ...
  window.location.href = '/cars-for-sale/' + (qs ? '?' + qs : '');
}
